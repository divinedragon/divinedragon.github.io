...
echo "[INFO] Validating Reads in Folder - $FOLDER_NAME"
...
for FILE in $(find . -maxdepth 1 -not -type d | cut -c3- | sort); do
    ...
    content_source=$(head -10 "$FILE" | tail -1 | grep '^## Source' | wc -l)

    # Check if the Notes section starts on Line 12
    ...
    [ "$content_source" -ne "1" ] && FAIL="${FAIL}\n\t🔴 'Source' content section is incorrect"
    [ "$content_notes" -ne "1" ] && FAIL="${FAIL}\n\t🔴 'Notes' content section is incorrect"
